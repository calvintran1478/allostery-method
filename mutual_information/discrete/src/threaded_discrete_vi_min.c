            ...
            for (int ai = 0; ai < num_attributes; ai++) {
                for (int aj = 0; aj < num_attributes; aj++) {
                    ...
                    if (h_x != -1 && h_y != -1) {
                        ...
                        // Take min information distacne between the variables
                        if (vi < data_matrix[i * num_rows + j]) {
 ...
 * Example:
 * ```
 * ./threaded_discrete_vi_min phi.csv,psi.csv 100 300000 1.0 360 output.csv 4
 * ```
 */
