...
/*
 * This function is used by each thread for computing mutual information values
 * and writing it to a shared data matrix.
 */
void *threaded_combine_mi(void *args) {
    ...
    if (joint_histogram_buffer == NULL) {
        fprintf(stderr, "Error allocating memory for MI computation");
        exit(1);
    }
...
/*
 * Performs a multithreaded computation of the mutual information matrix of the
 * given data using discretization.
 */
double *threaded_combine_mi_starter(double *data, int num_attributes, int num_rows, int num_cols, double bin_size, int num_bins, int num_threads) {
...
/*
 * Computes a mutual information matrix of the given data using discretization
 ...
 * Example:
 * ```
 * ./threaded_discrete_mi_avg phi.csv,psi.csv 100 300000 1.0 360 output.csv 4
 * ```
 */
int main(int argc, char *argv[]) {
    ...
    // Estimate MI
    double *data_matrix = threaded_combine_mi_starter(data_buffer, num_files, input_csv.num_rows, input_csv.num_cols, bin_size, (int) num_bins, num_threads);
