...
/* Struct definitions */
typedef struct combine_gcc_thread_args {
    double *data;
...
/*
 * This function is used by each thread for computing generalized correlation
 * coefficient values and writing it to a shared data matrix.
 */
void *threaded_combine_gcc(void *args) {
    // Extract thread arguments
    CombineGccThreadArgs *thread_args = (CombineGccThreadArgs *) args;
        ...
        for (int j = i; j < num_rows; j++) {
            ...
            for (int ai = 0; ai < num_attributes; ai++) {
                for (int aj = 0; aj < num_attributes; aj++) {
                    // Get self entropy of each variable attribute
                    ...
                    if (h_x != -1 && h_y != -1) {
                        ...
                        double mi =  h_x + h_y - h_x_y;
                        double corr = sqrt(1 - exp(-2 * mi));

                        // Sum up total correlation between the variables
                        data_matrix[i * num_rows + j] += corr;
                        num_correlations += 1;
                    }
                }
            }

            // Calculate average by dividing the total sum by the number of correlations
            if (num_correlations > 0) {
                data_matrix[i * num_rows + j] = data_matrix[i * num_rows + j] / num_correlations;
            }
...
/*
 * Performs a multithreaded computation of the generalized correlation
 * coefficient matrix of the given data using discretization.
 */
double *threaded_combine_gcc_starter(double *data, int num_attributes, int num_rows, int num_cols, double bin_size, int num_bins, int num_threads) {
    ...
    CombineGccThreadArgs thread_args = {
        .data = data,
    ...
    for (int i = 0; i < num_threads; i++) {
        pthread_create(&(tid[i]), NULL, &threaded_combine_gcc, (void *) &thread_args);
    }
 ...
 * Example:
 * ```
 * ./threaded_discrete_gcc_avg phi.csv,psi.csv 100 300000 1.0 360 output.csv 4
 * ```
 */
int main(int argc, char *argv[]) {
    ...
    // Estimate GCC
    double *data_matrix = threaded_combine_gcc_starter(data_buffer, num_files, input_csv.num_rows, input_csv.num_cols, bin_size, (int) num_bins, num_threads);
